	...
	//   let ai_toggle = document.querySelector('.ai-Toggle');
	  ...
	  const tlReferer = `https://${window.location.host}/`;
		...
		async function getGptResponse(content) {
			...
			const supabase = AI_option.supabase;
			...
			if (error) {
				console.log(error)
				startAI(`请求supabase出错了，请稍后再试。`);
				completeGenerate = true;
				return "";
			}

			if (data[0].summary) {
				sessionStorage.setItem('summary', JSON.stringify(data[0]));
				return data[0];
			}
			...
			try {
				...
				response = await fetch(AI_option.summary_url, {
					method: "POST",
					headers: {
			...
			console.log(res)
			// summaryId = res.id;
			sessionStorage.setItem('summary', JSON.stringify(res));
			return res;
		}
